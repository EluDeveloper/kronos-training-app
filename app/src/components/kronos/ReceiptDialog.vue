    ...
    <VCard v-if="receipt" class="kronos-card" rounded="xl">
      ...
      <VCardText class="pa-6">
        ...
        <VTable density="comfortable" class="mb-5">
          ...
          <tbody>
            <tr v-for="(line, index) in receipt.lines" :key="`${line.description}-${index}`">
              <td><strong>{{ line.quantity ? `${line.quantity} × ` : '' }}{{ line.description }}</strong><div v-if="line.quantity && line.unitPrice != null" class="text-caption text-medium-emphasis">{{ formatCurrency(line.unitPrice) }} por unidad</div></td>
