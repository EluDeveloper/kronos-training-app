...
function collectMembership(athleteId: string) {
  router.push({ path: '/pagos', query: { athleteId, period, collect: '1' } })
}
              ...
              <template v-else>
                <div v-if="overdueAthletes.length" class="mb-5">
                  ...
                  <VList bg-color="transparent" density="comfortable">
                    <VListItem v-for="athlete in overdueAthletes" :key="athlete.id" :title="athlete.profile.name" :subtitle="`Venció el día ${athlete.membership.paymentDay} · ${formatCurrency(athlete.membership.agreedAmount)}`" rounded="lg" @click="collectMembership(athlete.id)">
                      <template #append><VBtn size="small" color="error" variant="tonal">Cobrar</VBtn></template>
                    </VListItem>
                  </VList>
                </div>
                ...
                <div v-if="upcomingAthletes.length" class="mb-5">
                  ...
                  <VList bg-color="transparent" density="comfortable">
                    <VListItem v-for="athlete in upcomingAthletes" :key="athlete.id" :title="athlete.profile.name" :subtitle="`Vence el día ${athlete.membership.paymentDay}`" rounded="lg" @click="collectMembership(athlete.id)">
                      <template #append><VBtn size="small" variant="tonal">Preparar cobro</VBtn></template>
                    </VListItem>
                  </VList>
                </div>
