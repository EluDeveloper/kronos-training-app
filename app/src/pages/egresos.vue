...
<template>
  <PageHeader title="Egresos" eyebrow="Finanzas" description="Gastos operativos, compromisos y estado real de caja."><template #actions><VBtn prepend-icon="ri-add-line" @click="openCreate">Registrar egreso</VBtn></template></PageHeader>
  <VRow class="mb-2"><VCol cols="12" md="4"><MetricCard label="Pagado este mes" :value="formatCurrency(paidTotal)" icon="ri-money-dollar-circle-line" /></VCol><VCol cols="12" md="4"><MetricCard label="Por pagar" :value="formatCurrency(pendingTotal)" icon="ri-calendar-event-line" color="warning" /></VCol><VCol cols="12" md="4"><MetricCard label="Movimientos del mes" :value="thisMonth.length" icon="ri-file-list-3-line" color="secondary" /></VCol></VRow>
  <VCard class="kronos-card" rounded="xl"><VCardText><VRow><VCol cols="12" md="8"><VTextField v-model="search" label="Buscar egreso" prepend-inner-icon="ri-search-line" clearable /></VCol><VCol cols="12" md="4"><VSelect v-model="statusFilter" :items="[{title:'Pagado',value:'paid'},{title:'Pendiente',value:'pending'},{title:'Programado',value:'scheduled'}]" label="Estado" clearable /></VCol></VRow></VCardText>
    <VTable v-if="filtered.length" class="text-no-wrap"><thead><tr><th>FECHA</th><th>CATEGORÍA</th><th>DESCRIPCIÓN</th><th>MÉTODO</th><th>ESTADO</th><th>MONTO</th><th></th></tr></thead><tbody><tr v-for="expense in filtered" :key="expense.id"><td>{{ formatDate(expense.date) }}</td><td>{{ expense.category }}<div v-if="expense.subcategory" class="text-caption text-medium-emphasis">{{ expense.subcategory }}</div></td><td>{{ expense.description }}</td><td>{{ expense.method }}</td><td><VChip size="small" :color="statusColor(expense.status)">{{ statusLabel(expense.status) }}</VChip></td><td><strong>{{ formatCurrency(expense.amount) }}</strong></td><td><VBtn icon="ri-edit-line" variant="text" size="small" @click="openEdit(expense)" /><VBtn icon="ri-delete-bin-line" variant="text" size="small" color="error" @click="remove(expense)" /></td></tr></tbody></VTable><EmptyState v-else icon="ri-money-dollar-circle-line" title="Sin egresos" description="Registra gastos pagados o programados para conocer la caja real." /></VCard>
  <VDialog v-model="dialog" max-width="680"><VCard rounded="xl"><VCardTitle class="pa-6">{{ editing ? 'Editar egreso' : 'Registrar egreso' }}</VCardTitle><VCardText><VRow><VCol cols="12" md="4"><VTextField v-model="form.date" type="date" label="Fecha" /></VCol><VCol cols="12" md="4"><VTextField v-model="form.category" label="Categoría" /></VCol><VCol cols="12" md="4"><VTextField v-model="form.subcategory" label="Subcategoría" /></VCol><VCol cols="12"><VTextField v-model="form.description" label="Descripción" /></VCol><VCol cols="12" md="4"><VTextField v-model.number="form.amount" type="number" min="0" label="Monto" prefix="$" /></VCol><VCol cols="12" md="4"><VSelect v-model="form.method" :items="[{title:'Efectivo',value:'cash'},{title:'Transferencia',value:'transfer'},{title:'Tarjeta',value:'card'},{title:'Otro',value:'other'}]" label="Método" /></VCol><VCol cols="12" md="4"><VSelect v-model="form.status" :items="[{title:'Pagado',value:'paid'},{title:'Pendiente',value:'pending'},{title:'Programado',value:'scheduled'}]" label="Estado" /></VCol><VCol cols="12" md="6"><VTextField v-model="form.registeredBy" label="Registrado por" /></VCol><VCol cols="12" md="6"><VTextField v-model="form.receiptUrl" label="URL de comprobante (opcional)" /></VCol></VRow></VCardText><VCardActions class="pa-6 pt-0"><VSpacer /><VBtn variant="text" @click="dialog=false">Cancelar</VBtn><VBtn :loading="saving" @click="save">Guardar</VBtn></VCardActions></VCard></VDialog>
</template>
